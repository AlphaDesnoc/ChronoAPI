...
import com.chrono.chronoapi.builders.gui.opener.ChestInventoryOpener;
import com.chrono.chronoapi.builders.gui.opener.GuiOpener;
import com.chrono.chronoapi.builders.gui.opener.SpecialInventoryOpener;
...
public final class GuiManager
{
    ...
    private final Map<UUID, ChronoGui> inventories;
    private final Map<UUID, GuiContents> contents;

    private final List<GuiOpener> defaultOpeners;
    ...
    public void init()
    {
        ...
        new GuiManager.GuiTask().runTaskTimer(plugin, 1, 1);
    }

    public Optional<GuiOpener> findOpener(InventoryType type)
    {
        Optional<GuiOpener> opInv = this.openers.stream()
    ...
    public void registerOpeners(GuiOpener... openers)
    {
        this.openers.addAll(Arrays.asList(openers));
    }

    public List<Player> getOpenedPlayers(ChronoGui inv)
    {
    ...
    public Optional<ChronoGui> getInventory(Player p)
    {
        return Optional.ofNullable(this.inventories.get(p.getUniqueId()));
    }

    protected void setInventory(Player p, ChronoGui inv)
    {
        if (inv == null)
    ...
    public Optional<GuiContents> getContents(Player p)
    {
        return Optional.ofNullable(this.contents.get(p.getUniqueId()));
    }

    protected void setContents(Player p, GuiContents contents)
    {
        if (contents == null)
    ...
    public void handleInventoryOpenError(ChronoGui inventory, Player player, Exception exception)
    {
    ...
    public void handleInventoryUpdateError(ChronoGui inventory, Player player, Exception exception)
    {
        inventory.close(player);
    ...
    class InvListener implements Listener
    {
        ...
        public void onInventoryClick(InventoryClickEvent e)
        {
            ...
            if (clickedInventory == p.getOpenInventory().getTopInventory()) {
                ...
                final ChronoGui inv = inventories.get(p.getUniqueId());
    ...
    class GuiTask extends BukkitRunnable
    {

        @Override
        public void run() {
            new HashMap<>(inventories).forEach((uuid, gui) -> {
                final Player player = Bukkit.getPlayer(uuid);

                try {
                    gui.getProvider().update(player, contents.get(uuid));
                } catch (Exception e) {
                    handleInventoryUpdateError(gui, player, e);
                }
            });
        }
