...
public interface GuiContents
{
    ...
    GuiContents set(int row, int column, ClickableItem item);
    GuiContents set(SlotPos slotPos, ClickableItem item);

    GuiContents add(ClickableItem item);

    GuiContents fill(ClickableItem item);
    GuiContents fillRow(int row, ClickableItem item);
    GuiContents fillColumn(int column, ClickableItem item);
    GuiContents fillBorders(ClickableItem item);

    GuiContents fillRect(int fromRow, int fromColumn,
                               int toRow, int toColumn, ClickableItem item);
    GuiContents fillRect(SlotPos fromPos, SlotPos toPos, ClickableItem item);

    <T> T property(String name);
    <T> T property(String name, T def);

    GuiContents setProperty(String name, Object value);

    class Impl implements GuiContents
    {

        private ChronoGui inv;
        private UUID player;
        ...
        public Impl(ChronoGui inv, UUID player)
        {
            this.inv = inv;
            this.player = player;
        ...
        @Override
        public ChronoGui inventory() { return inv; }
        ...
        @Override
        public GuiContents set(int row, int column, ClickableItem item)
        {
            if(row >= contents.length)
                return this;
            if(column >= contents[row].length)
        ...
        @Override
        public GuiContents set(SlotPos slotPos, ClickableItem item)
        {
            return set(slotPos.getRow(), slotPos.getColumn(), item);
        }

        @Override
        public GuiContents add(ClickableItem item)
        {
            for(int row = 0; row < contents.length; row++) {
                for(int column = 0; column < contents[0].length; column++)
                {
        ...
        @Override
        public GuiContents fill(ClickableItem item)
        {
            for(int row = 0; row < contents.length; row++)
        ...
        @Override
        public GuiContents fillRow(int row, ClickableItem item)
        {
            if(row >= contents.length)
                return this;
        ...
        @Override
        public GuiContents fillColumn(int column, ClickableItem item)
        {
            for(int row = 0; row < contents.length; row++)
                set(row, column, item);
        ...
        @Override
        public GuiContents fillBorders(ClickableItem item)
        {
        ...
        @Override
        public GuiContents fillRect(int fromRow, int fromColumn, int toRow, int toColumn, ClickableItem item)
        {
        ...
        @Override
        public GuiContents fillRect(SlotPos fromPos, SlotPos toPos, ClickableItem item)
        {
        ...
        @Override
        public GuiContents setProperty(String name, Object value)
        {
